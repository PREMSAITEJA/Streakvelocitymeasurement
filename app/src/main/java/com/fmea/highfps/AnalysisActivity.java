...
public class AnalysisActivity extends AppCompatActivity {
    ...
    private int selectedInDirIdx = 0;
    private int selectedOutDirIdx = 2; // Default East
    ...
    private void showCalibrationDialog() {
        ...
        TextView lblInDir = new TextView(this);
        lblInDir.setText("IN direction:");
        Spinner spinInDir = new Spinner(this);
        ArrayAdapter<String> adapterIn = new ArrayAdapter<>(this, android.R.layout.simple_spinner_item, directions);
        adapterIn.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        spinInDir.setAdapter(adapterIn);
        spinInDir.setSelection(selectedInDirIdx);

        TextView lblOutDir = new TextView(this);
        lblOutDir.setText("OUT direction:");
        Spinner spinOutDir = new Spinner(this);
        ArrayAdapter<String> adapterOut = new ArrayAdapter<>(this, android.R.layout.simple_spinner_item, directions);
        adapterOut.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        spinOutDir.setAdapter(adapterOut);
        spinOutDir.setSelection(selectedOutDirIdx);
        ...
        dialogLayout.addView(lblInDir);
        dialogLayout.addView(spinInDir);
        dialogLayout.addView(lblOutDir);
        dialogLayout.addView(spinOutDir);
        ...
        new android.app.AlertDialog.Builder(this)
                ...
                .setPositiveButton("OK", (dialog, which) -> {
                    ...
                    selectedInDirIdx = spinInDir.getSelectedItemPosition();
                    selectedOutDirIdx = spinOutDir.getSelectedItemPosition();
                })
                .show();
    }
    ...
    private void setupInputControls() {
        ...
        sbRangeStart.setOnSeekBarChangeListener(new SimpleSeekBarListener() {
            ...
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                ...
                if (fromUser) {
                    if (progress > frameRangeEnd) {
                        sbRangeStart.setProgress(frameRangeEnd);
                    }
                    currentFrameIndex = frameRangeStart;
                    renderActivePreview();
                }
        ...
        sbRangeEnd.setOnSeekBarChangeListener(new SimpleSeekBarListener() {
            ...
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                ...
                if (fromUser) {
                    if (progress < frameRangeStart) {
                        sbRangeEnd.setProgress(frameRangeStart);
                    }
                    currentFrameIndex = frameRangeEnd;
                    renderActivePreview();
                }
                updateFrameRangeLabel();
            }
    ...
    private void updateFrameRangeLabel() {
        ...
        String startStr = String.valueOf(frameRangeStart + 1);
        String endStr = String.valueOf(frameRangeEnd + 1);
        String fullText = String.format(Locale.US,
                "Custom range: frame %s to %s of %d recorded  (%d frames will be analyzed)",
                startStr, endStr, frameFiles.size(), selectedCount);

        SpannableString ss = new SpannableString(fullText);

        int startIdx = fullText.indexOf("frame " + startStr) + "frame ".length();
        int endOfStartIdx = startIdx + startStr.length();
        ss.setSpan(new ClickableSpan() {
            ...
            public void updateDrawState(@NonNull android.text.TextPaint ds) {
                ...
                ds.setColor(Color.parseColor("#FFC107"));
                ds.setUnderlineText(true);
            }
        }, startIdx, endOfStartIdx, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);

        int midToIdx = fullText.indexOf(" to " + endStr) + " to ".length();
        int endOfEndIdx = midToIdx + endStr.length();
        ss.setSpan(new ClickableSpan() {
            @Override
            public void onClick(@NonNull View widget) {
                currentFrameIndex = frameRangeEnd;
        ...
        }, midToIdx, endOfEndIdx, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);

        lblFrameRange.setText(ss);
        lblFrameRange.setMovementMethod(LinkMovementMethod.getInstance());
        lblFrameRange.setHighlightColor(Color.TRANSPARENT);
    }
            ...
            for (File file : selectedFiles) {
                ...
                for (MatOfPoint contour : contours) {
                    if (!useArea || Imgproc.contourArea(contour) >= paramMinArea) {
                        double cx = 0, cy = 0, uVel = 0, vVel = 0;
                        ...
                        if (contour.rows() >= 5) {
                            ...
                            cx = ellipse.center.x;
                            cy = ellipse.center.y;
                            double angleRad = Math.toRadians(ellipse.angle - 90); // OpenCV angle starts from Y? Adjusting to match MATLAB
                            double majorAxis = Math.max(ellipse.size.width, ellipse.size.height);

                            double dx = (majorAxis / 2.0) * Math.cos(angleRad);
                            double dy = (majorAxis / 2.0) * Math.sin(angleRad);

                            Point p1 = new Point(cx - dx, cy - dy);
                            Point p2 = new Point(cx + dx, cy + dy);

                            Point dirVec = getDirectionVector(selectedOutDirIdx);
                            double dot = (p2.x - p1.x) * dirVec.x + (p2.y - p1.y) * dirVec.y;

                            Point tail, head;
                            if (dot >= 0) { tail = p1; head = p2; }
                            else { tail = p2; head = p1; }

                            uVel = ((head.x - tail.x) / targetScale) / 1000.0 / targetExposure;
                            vVel = ((head.y - tail.y) / targetScale) / 1000.0 / targetExposure;
                        } else {
                            Rect r = Imgproc.boundingRect(contour);
                            cx = r.x + (r.width / 2.0);
                            cy = r.y + (r.height / 2.0);
                            uVel = ((r.width / targetScale) / 1000.0) / targetExposure;
                            vVel = ((r.height / targetScale) / 1000.0) / targetExposure;
                        }
                ...
                // Modification: Removed per-frame result overlay saving.
                // We only need the averaged results and the final averaged image.
    ...
    private void exportAnalysisPackages() {
        ...
        pipelineExecutor.execute(() -> {
            try {
                ...
                // Modification: Removed per-frame vector CSV saving as requested.
    ...
    private Point getDirectionVector(int idx) {
