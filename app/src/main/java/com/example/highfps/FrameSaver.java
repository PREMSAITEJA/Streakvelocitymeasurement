...
public class FrameSaver {
	...
	private static void writeTagLE(FileOutputStream fos, int tag, int type, int count, int valueOrOffset) throws IOException {
		writeShortLE(fos, tag);
		writeShortLE(fos, type);
		writeIntLE(fos, count);
		writeIntLE(fos, valueOrOffset);
	}
}